...
impl HostJobScheduler {
    ...
    pub(crate) fn invoke_service_job(&self, request: Value) -> Result<Value, String> {
        let bytes = serde_json::to_vec(&request)
            .map_err(|e| format!("serialize engine.threading service-call request failed: {e}"))?;
        let blob = (self.call_service_v1)(
            CapabilityId::from(ENGINE_THREADING_GATEWAY_ID),
            MethodName::from(TASK_INVOKE_SERVICE_V1),
            Blob::from(bytes),
        )
        .into_result()
        ...
        serde_json::from_slice::<Value>(blob.as_slice())
            .map_err(|e| format!("engine.threading returned non-json service-call response: {e}"))
    }
}
